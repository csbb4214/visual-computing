...
void main(void) {
    ...
    vec3 illuminance = uLightSun.ambient * uMaterial.ambient * uMaterial.diffuse + uMaterial.emission;
    illuminance += uLightSun.color * brdf_blinn_phong(-uLightSun.direction, viewDir, normal, uMaterial.diffuse, uMaterial.specular, uMaterial.shininess);
    ...
    for(int i = 0; i < 4; i++) {
        ...
        illuminance += intensity * attenuation * uLightSpots[i].color * brdf_blinn_phong(lightDir, viewDir, normal, uMaterial.diffuse, uMaterial.specular, uMaterial.shininess);
    }
