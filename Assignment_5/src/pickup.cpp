...
Pickup pickupLoad(const std::string& filePath) {
    ...
    pickup.noEmissionTexture = textureCreateSingleColor(1, 1, {0.0f, 0.0f, 0.0f});
    ...
    for(const auto& obj : models) {
        if(obj.name == "Body") {
            pickup.partModel[Pickup::BODY] = obj;
            pickup.emissionTextures[Pickup::LIGHT_BL] = obj.material[3].map_emission;
        }
            ...
            pickup.emissionTextures[Pickup::LIGHT_BL] = obj.material[0].map_emission;
            ...
            pickup.emissionTextures[Pickup::LIGHT_BR] = obj.material[0].map_emission;
            ...
            pickup.emissionTextures[Pickup::LIGHT_FL] = obj.material[0].map_emission;
            ...
            pickup.emissionTextures[Pickup::LIGHT_FR] = obj.material[0].map_emission;
        } else throw std::runtime_error("[Pickup] unkown part name: " + obj.name);
    }

    pickup.wheelBase = 2.4f;
    pickup.width = 1.7f;
    pickup.turningDegPerMeter = calculateTurningAnglePerMeter(pickup.wheelBase, pickup.turningAngle, pickup.width);
...
void pickupDelete(Pickup &pickup) {
    ...
    textureDelete(pickup.noEmissionTexture);
...
void setEmission(Pickup &pickup, bool emission) {
    for (auto const &[part, color] : pickup.emissionColors) {
        ...
        pickup.partModel[part].material[0].map_emission = emission ? pickup.emissionTextures[part] : pickup.noEmissionTexture;
    }
}